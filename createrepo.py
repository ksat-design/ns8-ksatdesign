...
# Walk all directories on the given path: each path represents a package
...
for entry_path in glob.glob(path + '/*'):
    ...
    logo_path = os.path.join(entry_path, "logo.png")
    if os.path.isfile(logo_path) and imghdr.what(logo_path) == "png":
        metadata["logo"] = "logo.png"

    screenshot_dir = os.path.join(entry_path, "screenshots")
    if os.path.isdir(screenshot_dir):
        with os.scandir(screenshot_dir) as sdir:
